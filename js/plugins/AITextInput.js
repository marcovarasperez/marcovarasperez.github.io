...
(function() {
    ...
    AITextInput.showInput = function(interpreter) {
        ...
        var base = 'background:#2a2a4e;color:#e0c97f;border:1px solid #e0c97f;border-radius:4px;padding:6px 2px;font-size:12px;cursor:pointer;flex:1;margin:1px;min-width:24px;touch-action:manipulation;user-select:none;-webkit-user-select:none;';
        ...
        rows.forEach(function(row) {
            ...
            row.forEach(function(key) {
                var btn;
                if (key === 'CONFIRMAR') {
                    btn = makeBtn(key, 'background:#e0c97f;color:#1a1a2e;border:none;border-radius:4px;padding:8px 12px;font-size:12px;font-weight:bold;cursor:pointer;margin:1px;flex:2;touch-action:manipulation;user-select:none;-webkit-user-select:none;', finish);
                } else if (key === 'ESPACIO') {
                    btn = makeBtn(key, base + 'flex:3;', function() { if (text.length < 80) { text += ' '; updateScreen(); } });
                } else if (key === '⌫') {
                    btn = makeBtn(key, base + 'flex:1.5;', function() { text = text.slice(0, -1); updateScreen(); });
                } else {
                    btn = makeBtn(key, base, function() { if (text.length < 80) { text += key.toLowerCase(); updateScreen(); } });
                }
